 ...
 * duree, prix, statut ('en ligne'|'presentiel'|'hybride'), nb_inscrit.
 * Remplace l'ancienne version 100% basée sur lib/formations-data.ts
 * (données fictives, jamais connectées au backend) — ce qui expliquait
 * la perte du contexte de formation au clic sur "S'inscrire".
 */
interface Formation {
  id: string
  titre: string
...
function CourseCard({ course, index, isPopular }: { course: Formation; index: number; isPopular: boolean }) {
  return (
    <div
    ...
    >
      {/* Image Banner */}
      <div className="h-[180px] relative overflow-hidden bg-[#0d0d1a]">
        {course.image && (
          <Image
            src={course.image}
            alt={course.titre}
        ...
        <div className="absolute top-4 left-4">
          <span
            className={`inline-flex items-center gap-1.5 bg-[rgba(0,0,0,0.6)] backdrop-blur-sm border border-[rgba(201,162,39,0.4)] text-[#E8C050] text-[9px] font-bold tracking-[2px] uppercase rounded-full px-3 py-1.5 ${isPopular ? "bg-[#C9A227] text-[#0D2545] border-[#C9A227]" : ""}`}
          >
            {isPopular && (
            ...
            {isPopular ? "Populaire" : (course.niveau ?? "Tous niveaux")}
          </span>
        </div>
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <div className="inline-flex items-center gap-1.5 text-[10px] font-semibold tracking-[1px] text-[#C9A227] mb-3">
          ...
          {statutLabel[course.statut] ?? course.statut}
        </div>

        <p className="text-sm text-[#d0daf0] leading-relaxed mb-5 line-clamp-4">{course.description}</p>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-[rgba(255,255,255,0.06)] mt-auto">
          <div>
            ...
            <div className="font-serif text-xl font-bold text-[#C9A227]">{Number(course.prix).toLocaleString()} F</div>
          </div>
...
export function CoursesSection() {
  ...
  // "Populaire" = la formation avec le plus d'inscrits, calculé
  // dynamiquement plutôt qu'un champ fixe inexistant côté backend.
            ...
            <div className="relative">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {visible.map((course, index) => (
                  <CourseCard key={course.id} course={course} index={index} isPopular={course.id === mostPopularId} />
                ))}
              </div>
            </div>
